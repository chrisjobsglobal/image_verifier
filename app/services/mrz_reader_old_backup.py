...
class EnhancedMRZReaderService:
    ...
    def __init__(self):
        ...
        logger.info("� EnhancedMRZReaderService initialized")
    ...
    def _read_mrz_with_paddle(self, image: np.ndarray) -> Dict:
        ...
        try:
            if not self.paddle_ocr:
                logger.error("❌ PaddleOCR failed to initialize")
                return results
            
            logger.info(f"✅ PaddleOCR is ready, processing image...")
            ...
            for strategy_name, processed_img in strategies:
                if processed_img is None:
                    logger.warning(f"⚠️ Strategy '{strategy_name}' produced None image")
                    continue
                
                logger.info(f"🔍 Trying PaddleOCR with strategy: {strategy_name} (shape: {processed_img.shape})")
                
                # Run PaddleOCR (version 3.x doesn't use cls parameter)
                ...
                logger.info(f"📄 Found {len(ocr_results[0])} text lines with {strategy_name}")
                ...
                for idx, line in enumerate(ocr_results[0]):
                    if line and len(line) >= 2:
                        ...
                        logger.info(f"  📝 Line {idx}: '{text}' (len={len(text)}, conf={confidence:.2f})")
                        ...
                        if self._is_mrz_line(cleaned_text):
                            ...
                            logger.info(f"✓ MRZ-like line ({strategy_name}): {cleaned_text[:50]}... (conf: {confidence:.2f})")
    ...
    def _is_mrz_line(self, text: str) -> bool:
        ...
        # MRZ lines are typically 30-44 characters, but let's be lenient
        if len(text) < 15:
            return False
        ...
        # MRZ lines should have at least 40% valid characters and typical patterns
        is_mrz = ratio > 0.4 and has_uppercase and has_numbers
        
        if is_mrz:
            logger.debug(f"MRZ line detected: {text[:50]}... (len={len(text)}, ratio={ratio:.2f})")
